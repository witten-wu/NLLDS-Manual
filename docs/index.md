...
Last updated on Aug 8, 2024 by Yidu
...
- `Project Name`: Filled by administrators, supports clicking to jump to the subpage.
- `Survey ID`: Associated with the survey system automatically, supports clicking to jump to the survey page. Will be mentioned again later.
- `Manage By`: Specified by administrators.
- `Collaborator`: Specified by administrators.
- `Created By`: Record who created the project.
- `Created date`: Project create date.
- `Neuro`: Project have neuro test or not.

![](img/project_1.png)

### Subject

If you click the project name or double-click this project item, you will jump to the subpage which lists all the subject information of this project. The Description here shows the project description you filled before.

Click `Add Subject` button then you can add subjects for this project. The system will automatically generate the latest `Subject No.` in a specific format.

Here are descriptions of these fields:

- `Subject_No`: Automatically generated by system.
...
- `TaskFile`: Lists all the tasks of this project and supports clicking to jump to the file system to view/upload this subject's task data.
- `NeuroFile`: Supports clicking to jump to the file system to view/upload this subject's `Neuroimaging` data.

![](img/subject_1.png)

Click `Add Task` button then you can add tasks for this project. You can delete task by right click it. After created, it will show in the `TaskFile` column and you can click it to view/upload file. Please make sure the file name uploaded same as task name.

Here are descriptions of these fields:

- `Task Name`: Filled by you.
- `Descriptions`: Filled by you.
- `Created By`: Record who created the task.
- `Created date`: Task create date.

![](img/task_1.png)

Click `Check Status` button, the system will check the subject's survey answer submitted status and taskfile uploaded status. It will display a green tick when it's status are ready.
...
So how we can associate them? We need to set up the `Survey groups` same as our project name, and put our project survey into this survey group and active it, and then our system will automatically associate with it. If something wrong, the `Survey_ID` shows in the `Project` page will display `Not Found`.
...
First we need add the participants to the `Central participant management`. We don't need input too much sensitive information, only last name and email address can help us recognize the specific participant. 

![](img/survey_7.png)

Then we should assign this participant to the specific survey. After that, we can see them in the survey

![](img/survey_7_1.png)

![](img/survey_7_2.png)

Next, We need to assign the `Subject No.` to each participant here.

![](img/survey_8.png)

After that, each participant will need to fill in the access code (`Subject No.`) to access their Survey.
...
- The process of uploading files may be slow due to our server has I/O and network bandwidth limitations, please be patient. For large files, upload one by one will be better. It is not recommended to open multiple windows for uploading as it may occupy other people's network bandwidth.
